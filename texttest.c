...
int kerndist(uint16_t prev, uint16_t next) {
...
void blitchar(uint8_t *img, int stride, uint16_t ch) {
    for (int i = 0; i < 5; i++) {
...
int main(int argc, char *argv[]) {
    ...
    for (size_t i = 0; i < ntext; i++) {
        ...
        for (; i < ntext && text[i]; i++) {
            ...
            printf("line %d, char %d-%d dist: %d\n", h, (int)(i - 1), (int)i, dist);
            linew += dist;
            prev = text[i];
        }
        linew += 3;
    ...
    memset(img, 0, w * h);
    for (int line = 0, i = 0; i < ntext && line < nlines; line++, i++) {
        uint16_t prev = 0;
        int linew = 0;
        for (int j = i; j < ntext && text[j]; j++) {
        ...
        for (; i < ntext && text[i]; i++) {
            int dist = kerndist(prev, text[i]);
            x += dist;
            blitchar(img + y * w + x, w, text[i]);
            prev = text[i];
        }
    }
    FILE *f = fopen("img.raw", "w");
    fwrite(img, 1, w * h, f);
    fclose(f);
    return 0;
}
